...
## RP-COACH — Rebuild as a data-grounded narrator (DONE `20858ca`)
- [x] **Killed the mocked pattern** — removed the canned "Motivate me"/command pills.
- [x] **Data narrator shipped** — `ai.js` `askCoach()` assembles a real snapshot (training + top PRs + today's macros vs targets + body-weight trend + protocol adherence, from existing services) and prompts the model to cite specific figures, take corrections, and NEVER give medical/dosing advice (route to provider). Data-aware offline fallback. `CoachScreen` is now a free-text chat + data-grounded quick asks ("Am I winning?", "What should I train today?"…) with history for corrections. All client-side (coach edge fn is an unchanged thin proxy — no redeploy). Verified live: "Am I winning?" cites real workout count, streak, PRs, macro gap, weight trend. `getCoachResponse` (in-workout quips) left intact.
- [ ] **Explainable workout gen** (remaining) — when JustTalk generates a plan, anchor to recognizable templates (PPL, 5/3/1) with transparent progression, and surface AI-vs-local-fallback honestly in the UI. (Ties into RP-FUEL/Coach; do with the workout-gen pass.)
...
- [x] **Barcode scan UI — code done** (`0b84d1d`) — `expo-camera` added, `BarcodeScannerModal` scans → `lookupBarcode` → same portion picker as search (via `initialFood`); lazy-required so it degrades to a "needs rebuild" screen on an old binary. app.json: camera plugin + real permission string; bundle id aligned to `com.jackbowker.sayfit`, dropped `usesAppleSignIn` so a prebuild matches Personal-Team signing. **BLOCKED on Jack:** dev-client REBUILD to load the native camera module (`npx expo prebuild --clean` → open `ios/*.xcworkspace` in Xcode → set Personal Team → ⌘R). Free forever.
...
- [x] **Voice food logging** (`9348903`) — `ai.parseFoodText` (LLM → `{query,grams}` items) → `foodDb.resolveFoodItems` (USDA match + portion) → `VoiceFoodModal` review (edit grams, drop items, per-item + total macros) → log. Macros from the DB, not LLM guesses. Mic button in Fuel; lazy-requires speech module (degrades to typing). Verified parse live. (QuickAddMic→Fuel global routing still a small follow.)
- [x] **Adaptive TDEE** (`207f925`, =T15) — `energy.estimateTDEE()` back-calculates maintenance from intake vs weight trend; NutritionScreen "Adaptive maintenance" card with one-tap Maintain / Cut −500 (`targetsFromCalories`); Coach cites it. Verified math.
- [ ] **Fast-log floor.** Keep every log ≤2-3 taps (recents give one-tap re-log; still want previous-entry ghosting + QuickAddMic→Fuel routing). Friction — not feature count — kills all-in-one apps.
